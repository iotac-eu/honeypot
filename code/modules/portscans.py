...
class ScanLogger(object):
    ...
    def __init__(self, timeout, threshold, maxsize, daemon=True, logfile='/home/cowrie/cowrie/var/log/cowrie/cowrie.json'):
    ...
    def log_scan(self, scan, continuation=False):
        ...
        tojson = {}
        tojson["eventid"] = "iotac.honeypot.portscan";
        tojson["timestamp"] = get_timestamp();
        tojson["type"] = scan.type;
        if(scan.tcpflags_or):
            tojson["tcpflags"] = scan.tcpflags_or;
        tojson["src_ip"] = srcip;
        tojson["dst_ip"] = dstip;
        tojson["ports"] = ports;

        try:
            json_txt = str(json.dumps(tojson))
            print (json_txt)
    ...
    def log(self):
        ...
        # try:
        print ('[portscan] listening on %s: %s' % (pc.name, pc.filter))
        for ts, pkt in pc:
    ...
    def run_daemon(self):
        ...
        try:
            ...
            print (sys.stderr, "[portscan] fork #1 failed", e)
            sys.exit(1)
        ...
        #     print (sys.stderr, "[portscan] fork #2 failed", e)
        #     sys.exit(1)

        self.log()

    def run(self):
        print ("[portscan] initiated")
...
def main():
    ...
    s=ScanLogger(SCAN_TIMEOUT, WEIGHT_THRESHOLD, 8192, options.daemon, "")
    s.run()
