pipeline {
    ...
    stages {
      stage('Checkout') {
        ...
        // stage('Execute Unit Tests with Maven') {
        //     steps {
        ...
        // stage('Assemble .jar with Maven') {
        //     steps {
        //         echo 'Assemble .jar with Maven'
      ...
      //               sh 'docker run -d --label io.portainer.accesscontrol.teams=iotacdevs -p 8085:8080 --name "$APP_NAME" "$ARTIFACTORY_DOCKER_REGISTRY$DOCKER_IMAGE_TAG"'
